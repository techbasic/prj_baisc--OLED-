...
//		UartBufClear(&UartTxbuf);
		...
		switch(cmd)
		{
			case PC_PID_PITCH:
				...
				BufAddInt16((int16_t)((pitch_rate_pid->Kp * 100)));
				BufAddInt16((int16_t)((pitch_rate_pid->Ki * 100)));
				BufAddInt16((int16_t)((pitch_rate_pid->Kd * 100)));
#else 
				BufAddInt16((int16_t)((0* 100)));
				BufAddInt16((int16_t)((0* 100)));
				BufAddInt16((int16_t)((0* 100)));
#endif	
			
				BufAddInt16((int16_t)((pitch_angle_pid->Kp * 100)));
				BufAddInt16((int16_t)((pitch_angle_pid->Ki * 100)));
				BufAddInt16((int16_t)((pitch_angle_pid->Kd * 100)));
			...
			case PC_PID_ROLL:
				BufAdd8Chk(0x0C);	//len
#if  (PID_MODE != single_PID)
				BufAddInt16((int16_t)((roll_rate_pid->Kp * 100)));
				BufAddInt16((int16_t)((roll_rate_pid->Ki * 100)));
				BufAddInt16((int16_t)((roll_rate_pid->Kd * 100)));
			#else 
				BufAddInt16((int16_t)((0* 100)));
				BufAddInt16((int16_t)((0* 100)));
				BufAddInt16((int16_t)((0* 100)));
				...
				BufAddInt16((int16_t)((roll_angle_pid->Kp * 100)));
				BufAddInt16((int16_t)((roll_angle_pid->Ki * 100)));
				BufAddInt16((int16_t)((roll_angle_pid->Kd * 100)));
				break;
			case PC_PID_YAW:
				BufAdd8Chk(0x0C);	//len
 #if  (PID_MODE != single_PID)
			  BufAddInt16((int16_t)((yaw_rate_pid->Kp * 100)));
				BufAddInt16((int16_t)((yaw_rate_pid->Ki * 100)));
				BufAddInt16((int16_t)((yaw_rate_pid->Kd * 100)));
#else 
				BufAddInt16((int16_t)((0* 100)));
				BufAddInt16((int16_t)((0* 100)));
				BufAddInt16((int16_t)((0* 100)));
				...
				BufAddInt16((int16_t)((yaw_angle_pid->Kp * 100)));
				BufAddInt16((int16_t)((yaw_angle_pid->Ki * 100)));
				BufAddInt16((int16_t)((yaw_angle_pid->Kd * 100)));
				break;
			case PC_PID_ALT:
		/*		BufAdd8Chk(0x0C);	//len
				BufAddInt16((int16_t)((alt_vel_PID->Kp * 100)));
				BufAddInt16((int16_t)((alt_vel_PID->Ki * 100)));
				BufAddInt16((int16_t)((alt_vel_PID->Kd * 100)));
				
				BufAddInt16((int16_t)((alt_PID->Kp * 100)));
				BufAddInt16((int16_t)((alt_PID->Ki * 100)));
				BufAddInt16((int16_t)((alt_PID->Kd * 100)));
		*/		break;
...
void CommPCProcessCmd(void)
{
 ...
 #if  (PID_MODE != single_PID)
				pitch_rate_pid->Kp=(int16_t)(datBuf[0]<<8 | datBuf[1]) * 0.01f;
				pitch_rate_pid->Ki=(int16_t)(datBuf[2]<<8 | datBuf[3]) * 0.01f;
				pitch_rate_pid->Kd=(int16_t)(datBuf[4]<<8 | datBuf[5]) * 0.01f;
#endif
			
				pitch_angle_pid->Kp=(int16_t)(datBuf[6]<<8 | datBuf[7]) * 0.01f;
				pitch_angle_pid->Ki=(int16_t)(datBuf[8]<<8 | datBuf[9]) * 0.01f;
				pitch_angle_pid->Kd=(int16_t)(datBuf[10]<<8 | datBuf[11]) * 0.01f;
...
#if  (PID_MODE != single_PID)		
   			roll_rate_pid->Kp=(int16_t)(datBuf[0]<<8 | datBuf[1]) * 0.01f;
				roll_rate_pid->Ki=(int16_t)(datBuf[2]<<8 | datBuf[3]) * 0.01f;
				roll_rate_pid->Kd=(int16_t)(datBuf[4]<<8 | datBuf[5]) * 0.01f;
#endif
				roll_angle_pid->Kp=(int16_t)(datBuf[6]<<8 | datBuf[7]) * 0.01f;
				roll_angle_pid->Ki=(int16_t)(datBuf[8]<<8 | datBuf[9]) * 0.01f;
				roll_angle_pid->Kd=(int16_t)(datBuf[10]<<8 | datBuf[11]) * 0.01f;
...
//				gParamsSaveEEPROMRequset=1;
			...
			case PC_PID_YAW:	//yaw
 #if  (PID_MODE != single_PID)
			yaw_rate_pid->Kp=(int16_t)(datBuf[0]<<8 | datBuf[1]) * 0.01f;
				yaw_rate_pid->Ki=(int16_t)(datBuf[2]<<8 | datBuf[3]) * 0.01f;
				yaw_rate_pid->Kd=(int16_t)(datBuf[4]<<8 | datBuf[5]) * 0.01f;
#endif				
				yaw_angle_pid->Kp=(int16_t)(datBuf[6]<<8 | datBuf[7]) * 0.01f;
				yaw_angle_pid->Ki=(int16_t)(datBuf[8]<<8 | datBuf[9]) * 0.01f;
				yaw_angle_pid->Kd=(int16_t)(datBuf[10]<<8 | datBuf[11]) * 0.01f;
...
//		UartBufClear(&UartTxbuf);
		...
		//gyro
		BufAddInt16(imu.gyro[0] * 1000 );
		BufAddInt16(imu.gyro[1]  * 1000 );
		BufAddInt16(imu.gyro[2]  * 1000 );
		//mag
		BufAddInt16(0);
		BufAddInt16(0 );
		BufAddInt16(0 );
		...
		BufAddInt16(imu.gyroRaw[0] * 1000);
		BufAddInt16(imu.gyroRaw[1] * 1000 );
		BufAddInt16(imu.gyroRaw[2] * 1000 );
...
{
	Get_angle();//获取各种数据，赋值给结构体
		//50Hz Loop
		if(loop50HzFlag)//每20ms就传输一次。
		{
				loop50HzFlag=0;
